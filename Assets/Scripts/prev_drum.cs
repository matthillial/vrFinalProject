...
public class prev_drum : MonoBehaviour
{
    ...
    void Update()
    {
        if (clicked)
        {
            ...
            if (pad.GetComponent<drum_pad>().sample_choice == 0)
            {
                //Loop back to last drum choice
                pad.GetComponent<drum_pad>().sample_choice = pad.GetComponent<drum_pad>().samples.Length - 1;
                clicked = false;
            }
            else
            {
